...
export function HeroSection({ featured, playlist, familyQueue }: HeroSectionProps) {
  ...
  return (
    <section
      className="relative -mx-3 flex min-h-[430px] items-end overflow-hidden rounded-b-[34px] border border-white/[0.08] shadow-[0_20px_60px_rgba(0,0,0,0.35)] sm:mx-0 sm:min-h-[360px] sm:rounded-[32px] lg:min-h-[360px]"
      aria-live={isLive ? "polite" : "off"}
    >
      {/* Cover art — full bleed, aligned to section edges */}
      <div
      ...
      {/* Dark overlay — separate layer so it stays flush with hero edges on mobile */}
      <div
        className={cn(
          "pointer-events-none absolute inset-0 transition-opacity duration-500",
          isLive
            ? "bg-gradient-to-t from-[rgba(7,12,16,0.98)] via-[rgba(7,12,16,0.72)] to-[rgba(7,12,16,0.28)]"
            : "bg-gradient-to-br from-[rgba(7,12,16,0.96)] via-[rgba(7,12,16,0.78)] to-[rgba(7,12,16,0.22)] sm:bg-gradient-to-r sm:from-[rgba(7,12,16,0.96)] sm:via-[rgba(7,12,16,0.76)] sm:to-[rgba(7,12,16,0.12)]",
        )}
        aria-hidden
      />

      <div className="relative z-10 w-full p-6 sm:p-[34px]">
